#!/usr/bin/env bash
#
# Check that languages/woo-organization-accounts-pro.pot still describes the source.
#
...
DOMAIN="woo-organization-accounts-pro"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/woap-i18n.XXXXXX")"
trap 'rm -rf "$WORK"' EXIT

wp i18n make-pot "$PLUGIN_DIR" "$WORK/$DOMAIN.pot" \
	--slug="$DOMAIN" \
	--domain="$DOMAIN" \
	--package-name="WooCommerce Organization Accounts Pro" \
...
$committed = woap_msgids( $argv[1] );
$extracted = woap_msgids( $argv[2] );
