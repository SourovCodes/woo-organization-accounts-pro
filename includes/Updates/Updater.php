...
namespace WooOrgAccounts\Updates;
...
class Updater {
	 ...
	 */
	const SLUG = 'woo-organization-accounts-pro';

	/**
	 * The repository the releases are published from.
	 */
	const REPOSITORY = 'https://github.com/SourovCodes/woo-organization-accounts-pro';
	 ...
	 */
	const CACHE_KEY = 'woap_update_manifest';
	 ...
	 *
	 * The update check runs inside an admin request, so this is deliberately tight:
	 * an unreachable host must not hold an admin page open.
	 */
			...
			'sections'      => array(
				...
				'changelog'   => wpautop(
					sprintf(
						...
						esc_html__( 'What changed in each version is recorded with the release it shipped in: %s', 'woo-organization-accounts-pro' ),
						sprintf(
							'<a href="%1$s" target="_blank" rel="noopener noreferrer">%2$s</a>',
							esc_url( self::REPOSITORY . '/releases' ),
							esc_html__( 'release notes on GitHub', 'woo-organization-accounts-pro' )
						)
					)
				),
			),
	...
	public static function basename() {
		return plugin_basename( WOAP_PLUGIN_FILE );
	}
	...
	private function fetch() {
		 ...
		 */
		$url = apply_filters( 'woo_org_accounts_update_manifest_url', self::MANIFEST_URL );

		$response = wp_remote_get(
			$url,
			array(
				'timeout'    => self::REQUEST_TIMEOUT,
				'user-agent' => 'WooOrgAccountsPro/' . WOAP_VERSION . '; ' . home_url( '/' ),
	...
	private function normalise( $manifest ) {
		...
		return array(
			...
			'name'         => '' !== $text( 'name' ) ? $text( 'name' ) : 'WooCommerce Organization Accounts Pro',
