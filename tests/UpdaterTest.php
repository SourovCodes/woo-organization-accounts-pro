...
class UpdaterTest extends WP_UnitTestCase {
	...
	private function manifest( array $overrides = array() ) {
		return array_merge(
			array(
				'slug'         => 'woo-organization-accounts-pro',
				'plugin'       => 'woo-organization-accounts-pro/woo-organization-accounts-pro.php',
				'name'         => 'WooCommerce Organization Accounts Pro',
				'description'  => 'Turns WooCommerce into a strict organization-based B2B system.',
				'author'       => 'Sourov Biswas',
				'version'      => '9.9.9',
				'requires'     => '7.0',
				'requires_php' => '8.2',
				'url'          => Updater::REPOSITORY . '/releases/tag/v9.9.9',
				'package'      => Updater::PACKAGE_PREFIX . 'v9.9.9/woo-organization-accounts-pro-9.9.9.zip',
	...
	public function test_update_uri_header_points_at_the_repository() {
		$headers = get_file_data( WOAP_PLUGIN_FILE, array( 'update_uri' => 'Update URI' ) );
	...
	public function test_a_newer_release_is_offered() {
		...
		$this->assertSame( Updater::PACKAGE_PREFIX . 'v9.9.9/woo-organization-accounts-pro-9.9.9.zip', $update['package'] );
		$this->assertSame( 'woo-organization-accounts-pro', $update['slug'] );
		$this->assertSame( '8.2', $update['requires_php'] );
		$this->assertTrue( version_compare( $update['version'], WOAP_VERSION, '>' ) );
	}
	...
	public function test_a_current_version_is_still_answered() {
		$this->body = wp_json_encode( $this->manifest( array( 'version' => WOAP_VERSION ) ) );

		$update = $this->check();

		$this->assertIsArray( $update );
		$this->assertSame( WOAP_VERSION, $update['version'] );
		$this->assertFalse( version_compare( $update['version'], WOAP_VERSION, '>' ) );
	}
	...
	public function test_the_details_screen_is_answered() {
		$this->body = wp_json_encode( $this->manifest( array( 'name' => 'Acme <script>alert(1)</script>' ) ) );

		$args   = (object) array( 'slug' => 'woo-organization-accounts-pro' );
	...
	public function test_status_reports_being_current() {
		$this->seed_core_transient( 'no_update', WOAP_VERSION );

		$status = Updater::status();

		$this->assertSame( 'current', $status['state'] );
		$this->assertSame( WOAP_VERSION, $status['version'] );
	}
	...
	public function test_the_manual_check_is_hooked() {
		...
		$this->assertNotFalse( has_action( 'admin_post_woap_check_updates' ) );
	}
	...
	public function test_another_plugins_details_are_left_alone() {
		...
		$this->assertFalse( $updater->details( false, 'query_plugins', (object) array( 'slug' => 'woo-organization-accounts-pro' ) ) );
		$this->assertSame( 0, $this->requests );
	}
}
